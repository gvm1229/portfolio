...
A. 기존에 존재하는 프로젝트에 모델 파일을 추가하는 경우
- 기존 그룹을 선택하여 각종 정보를 계승했기 때문에 새로 입력해주어야 할 게 없음. 그 계승된 정보를 가지고 그저 파일을 업로드하기만 하면 끝.

B. 새로운 프로젝트의 모델 파일을 저장하는 경우
- 새로운 프로젝트 그룹을 생성해야 하기 때문에, 프로젝트의 이름, 베이스 모델 등 총 5개의 항목을 채워줘야 관련 폴더도 생성하여 원활한 정리가 가능.
