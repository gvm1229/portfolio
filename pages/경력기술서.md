...
**.pt 확장자를 가진, PyTorch에서 학습된 모델의 저장 형식이라던지, 모델의 전체 구조와 가중치를 저장하는 파일.**

-----

### 경험 및 성과
* **파일과 함께 최소 1개, 최대 5개 질문에 답만 하면 업로드 및 자동 배치 완료**

기존에 진행 중인 프로젝트에 모델 파일을 추가로 저장하는 경우
- 기존 그룹을 선택하기 때문에 가중치 1개만 따로 입력해주면 됨

새로운 프로젝트의 모델 파일을 저장하는 경우
- 가중치 뿐만 아니라 새로운 프로젝트의 이름과 용도 등 총 5개의 항목을 채워줘야 관련 폴더도 생성하여 원활한 정리가 가능.
