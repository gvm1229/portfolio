...
**딥러닝 파일로, PyTorch에서 학습된 모델의 전체 구조와 가중치를 저장하는 .pt 확장자 파일, 그리고 PyTorch 모델 파일을 TensorFlow 등에서 사용하기 위한 표준 중시의 .onnx 확장자 파일의 예시가 있음.**

-----

### 경험 및 성과
* **사용자 중심의 매우 간단한 업로드 과정**

A. 기존에 존재하는 프로젝트에 모델 파일을 추가하는 경우, 기존 그룹을 선택하여 각종 정보를 계승했기 때문에 새로 입력해주어야 할 게 없음. 그 계승된 정보를 가지고 그저 파일을 업로드하기만 하면 끝.

B. 새로운 프로젝트의 모델 파일을 저장하는 경우, 새로운 프로젝트 그룹을 생성해야 하기 때문에, 프로젝트의 이름, 베이스 모델 등 총 5개의 항목을 채워줘야 관련 폴더도 생성하여 원활한 정리가 가능.

* **파일들의 체계적인 정리**

NAS에 직접 접속하여 AI 모델 파일을 수작업으로 관리해서 파일 명명 규칙과 파일 그룹이 일관되지 않아 파일 찾기 및 협업이 어려웠던 기존 방식과 달리,
<br/>새로운 시스템을 통해 파일들은 업로드 즉시 자동 분류가 되며, AI 모델 파일 업로드·수정·삭제 시 중복 파일·잘못된 분류 그룹과 같은 문제의 발생을 원천 차단

* **Storybook 을 통한 컴포넌트 관리로 유지보수 용이하도록 기반 마련**

각 컴포넌트를 모듈화 및 Storybook 에 모듈 인자들을 명시 및 테스트 가능하도록 해 변경이 용이.

명료하고 자세한 주석을 통해 코드 유지보수 용이.

* **누구든 원클릭으로 간편하게 빌드할 수 있도록 프로세스 간소화**

Docker 를 통해 빌드 과정을 다 정리해두었고, Shell 스크립트로 그걸 실행시키기만 하면 Docker 이미지가 만들어지는 방식.

스크립트 실행 -> Docker 이미지와 배포 스크립트를 서버에 업로드 -> 배포 스크립트 실행

의 간편한 과정을 통해 Docker 컨테이너로 배포 완료.
...
| **참가 인원, 역할, 기여도** | 프로젝트 리드 및 사수: 1명 (60%)<br/>**프로젝트 보조 및 프런트엔드: 본인 1명 (40%)** |
